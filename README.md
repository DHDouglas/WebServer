...
- 共考虑四种情况：(短连接/长连接)  x   (有/无磁盘IO)
	- 为确定磁盘I/O的影响，设置了一个特殊URL `/benchmark`，其将直接回发 "Hello Word!" 消息体，不请求文件；
	- 请求 `/test.txt` 时，将访问磁盘上资源路径下的 `resources/test.txt` 文件，以其作为消息体回传。（该文件内容仅为  `"This is a test file."`）
...
| -------- | ------- | ---------- | ------------------ | ------------------- | ------------------- |
| 短连接   | ×       | 12B        | 2 W                | **1.95 W**          | 1.69 W              |
| 短连接   | ×       | **5KB**    |                    |                     |                     |
| 短连接   | √       | 20B        | 1.3 W              | 1.3 W               | 1.24 W              |
| 长连接   | ×       | 12B        | 9.8 W              | **8.8 W**           | 7.1 W               |
| 短连接   | ×       | **5KB**    |                    |                     |                     |
| 长连接   | √       | 20B        | 3 W                | 2.8 W               | 2.7 W               |
...
初期阶段曾向多个 WebServer 小项目学习, 最后转向了muduo。 
